...
function isRecentlyAnnounced(announceDate) {
  const d = parseFlex(announceDate);
  if (!d) return false;
...
export async function getStaticPaths() {
  const paths = noticesData.notices.map((n) => ({ params: { id: String(n.id) } }));
  return { paths, fallback: false };
}

export async function getStaticProps({ params }) {
  const notice = noticesData.notices.find((n) => String(n.id) === String(params.id));
...
function MiniCalendar({ startDate, endDate, winnerDate }) {
  ...
  const startOffset = (first.getDay() + 6) % 7; // 월요일 시작 기준
  ...
  function getCellType(d) {
    ...
    if (winner && new Date(year, month, d).toDateString() === winner.toDateString()) {
      return "winner";
    }
    return null;
  }
...
export default function NoticeDetail({ notice }) {
  ...
  useEffect(() => {
    ...
    setIsNew(isRecentlyAnnounced(notice.announce_date));
  }, [notice.announce_date]);
  ...
  return (
    <>
      <Head>
        <title>{notice.title} - 청약나라</title>
        <meta name="description" content={`${notice.source_agency} ${notice.title} 공고 상세 정보입니다.`} />
      </Head>
        ...
        <div className="layout">
          ...
          <div className="main-col">
            <div className="badge-row">
              ...
              {notice.source_agency && <span className="badge agency">{notice.source_agency}</span>}
