...
function parseGhListHtml(html, sourceLabel) {
  ...
  const rows = [];
  // 🔍 임시 디버그: 이 페이지에서 table.board_tbl 자체를 찾았는지, tr이 몇 개인지 확인
  const tableFound = $("table.board_tbl").length;
  const trCount = $("table.board_tbl tbody tr").length;
  console.log(`[디버그] ${sourceLabel} 파싱: table.board_tbl 발견=${tableFound}개, tr=${trCount}개`);

  $("table.board_tbl tbody tr").each((_, el) => {
    const tds = $(el).find("td");
    if (tds.length < 10) {
      console.log(`[디버그] ${sourceLabel} 행 건너뜀 (td개수=${tds.length}, 10 미만)`);
      return; // 형식이 다른 행은 건너뜀
    }
    ...
    if (!pbancNo || !title) {
      console.log(
        `[디버그] ${sourceLabel} 행 건너뜀 (pbancNo=${pbancNo}, title="${title}") td[2] HTML: ${$(tds[2]).html()}`
      );
      return;
    }
...
async function fetchGhScrapeAll({ maxPagesPerBoard = 5 } = {}) {
  ...
  console.log(`[디버그] GH 스크래핑 원본 샘플 3건:`, JSON.stringify(all.slice(0, 3), null, 2));
  return all;
}
