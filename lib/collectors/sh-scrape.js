...
// 진짜 "주택임대 공고 및 공지" 게시판 (app 도메인, m_247)
...
function parseShListHtml(html, pageNum) {
  ...
  trList.each((index, element) => {
    ...
    const rawTitle = $a.clone().find(".icoNew").remove().end().text();
    ...
    const seqMatch = onclickAttr.match(/getDetailView\('(\d+)'\)/);
...
async function fetchShDetail(seq) {
  ...
  const fullDateRangeMatch = contText.match(
    /(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})\.\([^)]*\)\s*\d{1,2}:\d{2}\s*~\s*(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})\.\([^)]*\)\s*\d{1,2}:\d{2}/
  );
  const sameYearRangeMatch = contText.match(
    /(?:인터넷\s*접수|접수기간|접수일)\s*[:：]?\s*(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})\.\([^)]*\)\s*\d{1,2}:\d{2}\s*~\s*(\d{1,2})\.\s*(\d{1,2})\.\([^)]*\)\s*\d{1,2}:\d{2}/
  );

  if (fullDateRangeMatch) {
    apply_start_date = `${fullDateRangeMatch[1]}-${fullDateRangeMatch[2].padStart(2, "0")}-${fullDateRangeMatch[3].padStart(2, "0")}`;
    apply_end_date = `${fullDateRangeMatch[4]}-${fullDateRangeMatch[5].padStart(2, "0")}-${fullDateRangeMatch[6].padStart(2, "0")}`;
  } else if (sameYearRangeMatch) {
    const year = sameYearRangeMatch[1];
    apply_start_date = `${year}-${sameYearRangeMatch[2].padStart(2, "0")}-${sameYearRangeMatch[3].padStart(2, "0")}`;
    apply_end_date = `${year}-${sameYearRangeMatch[4].padStart(2, "0")}-${sameYearRangeMatch[5].padStart(2, "0")}`;
  }
  ...
  // 🆕 첨부파일: 실제 다운로드 버튼은 자바스크립트라 URL을 못 얻지만,
  //    "미리보기" 링크(htmlConverter.do)는 실제 주소라서 이걸 대신 사용
  const attachment_urls = [];
  $("a.icoView").each((_, el) => {
    const $link = $(el);
    const href = $link.attr("href");
    if (!href) return;
    const fullUrl = href.startsWith("http") ? href : `${SH_BASE_URL}${href}`;
    const $row = $link.closest("tr");
    const name = $row.find(".btnAttach").first().text().replace(/\s+/g, " ").trim() || null;
    attachment_urls.push({ url: fullUrl, name, label: "미리보기" });
  });

  // 🆕 주소: 본문 안에 "소재지/공급위치/대상지" 표현이 있으면 뽑아봄.
  //    공고 종류마다 표현이 달라서 못 찾으면 null (완벽하지 않음, 최선의 시도)
  const addressMatch = contText.match(/(?:소재지|공급위치|대상지)\s*[:：]\s*([^\s].{2,60}?)(?=\s{2,}|■|○|$)/);
  const address_detail = addressMatch ? addressMatch[1].trim() : null;
...
function normalizeShScraped(row, index) {
  ...
  const formattedTitle = cleanTitle.startsWith("[SH]") ? cleanTitle : `[SH] ${cleanTitle}`;
  return {
    ...
    move_in_date: null,
    contact_phone: null,
    contact_address: row.address_detail ?? null,
    contact_note: null,
    etc_note: null,
    status: "진행중",
    special_supply_tags: [],
