...
// 형식 1: "2026. 9. 7.(월) 09:00 ~ 2026. 9. 9.(수) 18:00" — 양쪽 다 연도 표기
const fullDateRangeMatch = contText.match(
  /(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})\.\([^)]*\)\s*\d{1,2}:\d{2}\s*~\s*(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})\.\([^)]*\)\s*\d{1,2}:\d{2}/
);

// 형식 2: "인터넷 접수: 2026. 9. 9.(수) 10:00 ~ 9. 11.(금) 17:00" — 종료일에 연도 생략(시작일과 같은 연도)
