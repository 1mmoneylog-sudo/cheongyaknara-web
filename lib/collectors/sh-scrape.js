...
function parseShListHtml(html, pageNum) {
  ...
  trList.each((index, element) => {
    ...
    // ✅ getDetailView('숫자') 패턴만 정확히 매칭 (다른 종류 링크는 seq 없음으로 처리)
    const seqMatch = onclickAttr.match(/getDetailView\('(\d+)'\)/);
    const seq = seqMatch ? seqMatch[1] : null;

    // ✅ seq를 못 찾은 행(외부 링크 등)은 상세페이지 수집 대상에서 아예 제외
    if (!seq) return;
...
function normalizeShScraped(row, index) {
  // ✅ 이미 정제된 title을 쓰되, 혹시 남은 공백까지 한 번 더 정리
