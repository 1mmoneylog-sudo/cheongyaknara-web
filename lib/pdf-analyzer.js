...
//
// ✅ 수정 1: Gemini 모델명을 gemini-2.5-flash → gemini-3.6-flash로 변경
//    (구글 API가 에러 메시지에서 직접 안내한 최신 모델명)
// ✅ 수정 2: PDF 다운로드 응답이 실제로 PDF가 맞는지 확인하는 코드 추가.
//    PDF는 항상 파일 맨 앞이 "%PDF"로 시작하는데, 이게 아니면 LH 서버가
//    PDF 대신 오류 페이지나 접근거부 메시지를 보낸 것이므로, 그 실제 내용을
//    로그에 남겨서 "왜 실패하는지" 다음 실행에서 바로 보이게 함.
//
...
const GEMINI_MODEL = "gemini-3.6-flash"; // ⚠️ 구글이 에러에서 안내한 최신 모델명으로 변경
...
async function downloadPdfText(pdfUrl) {
  ...
  // ✅ PDF 파일은 항상 앞 4바이트가 "%PDF"로 시작한다. 이게 아니면
  //    진짜 PDF가 아니라 다른 내용(오류 페이지, 로그인 요구 등)이 온 것.
